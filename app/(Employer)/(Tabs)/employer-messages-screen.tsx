...
export default function EmployerMessagesScreen() {
  ...
  const handleOpenChat = (conversationId: string) => {
    router.push(`/chat/${conversationId}?role=employer`);
  };
      ...
      <View className="px-4 py-2 mt-2">
        {activeTab === 'messages' ? (
          conversations.map((c) => (
            ...
            >
              <Image source={userIcon} className="w-12 h-12 mr-3" />

              <View>
                <Text style={{ fontFamily: 'Lexend-Bold' }} className="text-lg">{c.participants.jobseeker}</Text>
